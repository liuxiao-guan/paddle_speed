...
def score_evaluate(cache_dic, tokens, current) -> paddle.Tensor:
    '''
    ...
    if cache_dic['cache_type'] == 'random':
        score = paddle.rand(tokens.shape[0], tokens.shape[1], device=tokens.device)

    elif cache_dic['cache_type'] == 'straight':
        score = paddle.ones(tokens.shape[0], tokens.shape[1]).to(tokens.device)
        ...
        score1 = paddle.rand(int(tokens.shape[0]*0.5), tokens.shape[1])
        score1 = paddle.cat([score1, score1], dim=0).to(tokens.device)
