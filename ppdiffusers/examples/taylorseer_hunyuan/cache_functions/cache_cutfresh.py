...
def cache_cutfresh(cache_dic, tokens, current):
    ...
    tokens: paddle.Tensor, the input tokens to be cut.
    ...
    fresh_ratio = paddle.clip(paddle.to_tensor(fresh_ratio, dtype=tokens.dtype, place=tokens.place), min=0, max=1)
    #paddle.clip(paddle.to_tensor(fresh_ratio, dtype=tokens.dtype, place=tokens.place), min=0, max=1)
    ...
    ###                                                                src = paddle.zeros_like(fresh_indices, dtype=paddle.int, device=fresh_indices.device))
    ...
    fresh_tokens = paddle.gather(input = tokens, dim = 1, index = fresh_indices_expand)
...
def local_selection_with_bonus(score, bonus_ratio, grid_size=2):
    ...
    mask = paddle.zeros_like(score_reshaped)
