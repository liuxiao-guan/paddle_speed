...
def support_set_selection(x: paddle.Tensor, fresh_ratio: float, base_ratio: float, current: Dict, cache_dic: Dict) -> paddle.Tensor:
    ...
    random_indices = paddle.randperm(N, device=x.device)
    ...
    similarity = paddle.einsum('bnd,bmd->bnm', base_tokens, x_norm)
    ...
    #indices = paddle.cat([base_indices, other_indices[min_indices]], dim=-1)
    indices = paddle.cat([base_indices.expand(B, -1), min_indices], dim=-1) #+ selection_start
