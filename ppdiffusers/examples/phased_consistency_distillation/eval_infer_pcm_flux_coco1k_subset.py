...
for path_to_lora in path_lora:
    ...
    parts = path_to_lora.strip("/").split("/")  # 去掉开头的空字符串

    if "checkpoint-" in parts[-2]:
        checkpoint_version = parts[-2].split("-")[-1]
        checkpoint_date = parts[-3].split("_")[-1]
    else:
        checkpoint_version = "latest"
        checkpoint_date = parts[-2].split("_")[-1]
