...
save_root = "/root/paddlejob/workspace/env_run/output/gxl/pcm_eval_results_flux_coco10k"
...
prompt_path = "/root/paddlejob/workspace/env_run/gxl/output/PaddleMIX/inf_speed/coco10k/all_prompts.pkl"
...
# 每个进程保存到不同目录
save_dir = os.path.join(save_root, f"{save_dir_name}_rank_{local_rank}")
if not os.path.exists(save_dir):
    os.makedirs(save_dir)
import pickle
with open('/root/paddlejob/workspace/env_run/gxl/output/PaddleMIX/inf_speed/coco10k/all_prompts.pkl', 'rb') as f:
...
for i, prompt in tqdm(enumerate(captions[local_rank::world_size]), 
                     total=len(captions[local_rank::world_size])):
    print(f"{i}: {prompt}")
    with paddle.no_grad():
        result_image = pipe(
            prompt=prompt,
