...
import paddle
from paddle.utils._pytree import tree_map, tree_flatten, tree_unflatten
from .module_tracker import ModuleTracker
from typing import Any, Optional, Union, TypeVar, Callable
from collections.abc import Iterator
from typing_extensions import ParamSpec
from collections import defaultdict
from paddle.utils._python_dispatch import paddleDispatchMode
...
def register_flop_formula(targets, get_raw=False) -> Callable[[Callable[_P, _T]], Callable[_P, _T]]:
    def register_fun(flop_formula: Callable[_P, _T]) -> Callable[_P, _T]:
        ...
        def register(target):
            if not isinstance(target, paddle._ops.OpOverloadPacket):
                raise ValueError(
                    f"register_flop_formula(targets): expected each target to be "
                    f"OpOverloadPacket (i.e. paddle.ops.mylib.foo), got "
        ...
        paddle.utils._pytree.tree_map_(register, targets)
...
def _offsets_to_lengths(offsets, max_len):
    ...
    """
    from paddle._subclasses.fake_tensor import FakeTensor
    from paddle._subclasses.functional_tensor import FunctionalTensor
...
class FlopCounterMode:
    ...
    It does this using a ``paddleDispatchMode``.
    ...
    def __init__(
            self,
            mods: Optional[Union[paddle.nn.Module, list[paddle.nn.Module]]] = None,
            depth: int = 2,
            display: bool = True,
...
class _FlopCounterMode(paddleDispatchMode):
    def __init__(self, counter: FlopCounterMode):
        self.counter = counter

    def __paddle_dispatch__(self, func, types, args=(), kwargs=None):
        kwargs = kwargs if kwargs else {}

        # Skip ops from non-standard dispatch_sizes_strides_policy such as NJT
        if func in {paddle.ops.aten.is_contiguous.default,
                    paddle.ops.aten.is_contiguous.memory_format,
                    paddle.ops.aten.is_strides_like_format.default,
                    paddle.ops.aten.is_non_overlapping_and_dense.default,
                    paddle.ops.aten.size.default,
                    paddle.ops.aten.sym_size.default,
                    paddle.ops.aten.stride.default,
                    paddle.ops.aten.sym_stride.default,
                    paddle.ops.aten.storage_offset.default,
                    paddle.ops.aten.sym_storage_offset.default,
                    paddle.ops.aten.numel.default,
                    paddle.ops.aten.sym_numel.default,
                    paddle.ops.aten.dim.default,
                    paddle.ops.prim.layout.default}:

            return NotImplemented

        # If we don't have func in flop_registry, see if it can decompose
        if func not in self.counter.flop_registry and func is not paddle.ops.prim.device.default:
            with self:
