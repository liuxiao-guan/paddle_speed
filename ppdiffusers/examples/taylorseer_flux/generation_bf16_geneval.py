...
def parse_args():
    ...
    parser.add_argument(
        "--saved_path",
        type=str,
        default='/root/paddlejob/workspace/env_run/gxl/output/PaddleMIX/inf_speed_bf16/GenEval',
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="number of images to generate for one prompt",
    )
    parser.add_argument(
        "--batch_size",
        type=int,
...
if __name__ == '__main__':
    ...
    if args.dataset == "coco10k":
        ...
        with open('/root/paddlejob/workspace/env_run/gxl/paddle_speed/ppdiffusers/examples/taylorseer_flux/prompts/DrawBench.txt', 'r', encoding='utf-8') as f:
            all_prompts = [line.strip() for line in f if line.strip()]
              ...
              # Load prompts
            with open("/root/paddlejob/workspace/env_run/gxl/paddle_speed/ppdiffusers/examples/taylorseer_flux/prompts/evaluation_metadata.jsonl") as fp:
    ...
    if args.origin == True:
        ...
        if args.dataset == "coco10k":
            ...
            saved_path = os.path.join(args.saved_path,"origin_50steps_geneval")
    ...
    if args.pab == True:
        ...
        if args.dataset == "coco10k":
            ...
            saved_path = os.path.join(args.saved_path,"pab_R100-950_N10_geneval")
    ...
    if args.taylorseer == True:
        ...
        if args.dataset == "coco10k":
            ...
            saved_path = os.path.join(args.saved_path,"taylorseer_N2O1_geneval")
    # 加入teacache 方法的
    if args.teacache == True :
        ...
        if args.dataset == "coco10k":
            ...
            saved_path = os.path.join(args.saved_path,"teacache0.40_geneval")
    ...
    if args.firstblock_predicterror_taylor == True:
        ...
        if args.dataset == "coco10k":
            ...
            saved_path = os.path.join(args.saved_path,"firstblock_predicterror_taylor0.03_geneval")
    ...
    for index, metadata in enumerate(tqdm(metadatas)):
        outpath = os.path.join(saved_path, f"{index:0>5}")
        os.makedirs(outpath, exist_ok=True)
        prompt = metadata['prompt']
        n_rows = batch_size = args.batch_size
        ...
        sample_count = 0
        for n in trange((args.n_samples + batch_size - 1) // batch_size, desc="Sampling one"):
            image = pipe(
                prompt=prompt,
                height=1024,
                width=1024,
