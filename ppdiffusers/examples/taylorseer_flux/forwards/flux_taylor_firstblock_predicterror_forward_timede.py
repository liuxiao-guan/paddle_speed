...
def Taylor_firstblock_predicterror_timede_Forward(
        self,
        sigmas: None,
        ...
        if self.enable_teacache:
            ...
            if self.cnt > 4:
                ...
                time_depen = 1-math.exp(-(1-sigmas[self.cnt])**2/sigmas[self.cnt]**2)
                # print(f"time_depen:{time_depen}")
                time_threshold =max(self.threshold*time_depen,0.03)
                # print(f"time_threshold:{time_threshold}")
                can_use_cache = self.predict_loss < time_threshold
