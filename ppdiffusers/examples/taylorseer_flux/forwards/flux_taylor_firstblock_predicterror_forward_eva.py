...
def Taylor_firstblock_predicterror_Forward_eva(
        self,
        ...
        if self.enable_teacache:
            ...
            if self.cnt > 4:
                ...
                self.error_firstblock.append(self.predict_loss.item())
                ...
                self.error_firstblock.append(0)
        ...
        if self.enable_teacache:
            if not should_calc:
                ...
                ture_hidden_states = hidden_states[:, encoder_hidden_states.shape[1] :, ...]
                hidden_states = step_taylor_formula(cache_dic=cache_dic, current=current)
                self.error_output.append((hidden_states - ture_hidden_states).abs().mean() / hidden_states.abs().mean())
                ...
                pre_hidden_states = step_taylor_formula(cache_dic=cache_dic, current=current)
                ...
                self.error_output.append(((hidden_states - pre_hidden_states).abs().mean()/hidden_states.abs().mean()).item())
