...
def Taylor_timeemb_predicterror_Forward(
        self,
        ...
        if self.enable_teacache:
            ...
            pre_modulated_inp = firstblock_taylor_formula(cache_dic=cache_dic, current=current)
            ...
            if self.cnt > 4:
                self.predict_loss = (pre_modulated_inp - modulated_inp).abs().mean()/modulated_inp.abs().mean()
                can_use_cache = self.predict_loss < self.threshold
                if can_use_cache == False:
                    # 要用block得输入来预测
                    current['block_activated_steps'].append(current['step'])
                    
                    firstblock_derivative_approximation(cache_dic=cache_dic, current=current, feature=modulated_inp)
